...
const Icon = {
  ...
  unlock: <svg width="15" height="15" viewBox="0 0 16 16" fill="none"><path d="M3 8h10M9 4l4 4-4 4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg>,
};
...
export default function LockScreen({ onUnlock }) {
  ...
  const handleUnlock = async () => {
    ...
    try {
      ...
      await api.post("/auth/verify-lock-code", { lockCode: code });
      setCode("");
      onUnlock();
    } catch (err) {
      setError(err.response?.data?.message || "קוד שגוי, נסה שוב");
      setShake(true);
  ...
  return (
    <div style={{
      ...
      {/* Decorative circles */}
      {[
        { size: 400, top: -120, right: -100, op: 0.06 },
        { size: 250, bottom: -60, left: -80,  op: 0.08 },
        { size: 150, top: "30%", left: "10%", op: 0.05 },
        { size: 100, bottom: "20%", right: "8%", op: 0.06 },
      ].map((c, i) => (
        <div key={i} style={{
      ...
      >
        ...
        {/* Brand footer */}
        <div style={{ marginTop: 22, textAlign: "center", fontSize: 11, color: "#ddd" }}>
          Yasmine Ledger · מערכת מאובטחת
        </div>
      </div>
    </div>
  );
