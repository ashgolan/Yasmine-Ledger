...
function NewItemModal({ open, onClose, onSaved }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 1000, background: "rgba(0,0,0,0.28)", display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}
      onClick={handleClose}>
      <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 400, padding: "26px 28px 22px", direction: "rtl" }}
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          {error && (
            <div style={{ background: C.red.bg, color: C.red.text, border: `0.5px solid ${C.red.border}`, borderRadius: 8, padding: "8px 12px", fontSize: 12, fontWeight: 600 }}>
              {error}
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 20 }}>
          <button onClick={handleSave} disabled={loading} style={{
            ...
            padding: "9px 22px", fontSize: 13, fontWeight: 600, cursor: loading ? "not-allowed" : "pointer",
            opacity: loading ? 0.7 : 1,
          }}>
            {loading ? "שומר..." : "הוסף פריט"}
          </button>
          <button onClick={handleClose} disabled={loading} style={{
            background: "#f5f5f5", color: "#555", border: "none", borderRadius: 9,
...
export default function ItemsPage() {
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: 900, margin: "0 auto", display: "flex", flexDirection: "column", gap: 16, animation: "fadeIn 0.3s ease" }}>
        ...
        <div style={{ background: "#fff", border: "0.5px solid #e8e8e8", borderRadius: 14, padding: "22px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 12 }}>
          <div>
            ...
            <div style={{ fontSize: 22, fontWeight: 700, color: "#1a1a1a" }}>פריטים</div>
            <div style={{ fontSize: 13, color: "#aaa", marginTop: 4 }}>ניהול מאגר הפריטים וקטגוריות</div>
          </div>
        ...
        {/* ── Search ── */}
        <div style={{ background: "#fff", border: "0.5px solid #e8e8e8", borderRadius: 14, padding: "13px 20px", display: "flex", alignItems: "center", gap: 10 }}>
          <span style={{ color: "#bbb", flexShrink: 0 }}>{Icon.search}</span>
          <input
            value={search}
            onChange={e => setSearch(e.target.value)}
            placeholder="חפש לפי שם פריט או קטגוריה..."
            style={{ flex: 1, border: "none", outline: "none", fontSize: 14, color: "#1a1a1a", background: "transparent", fontFamily: "inherit" }}
          />
          {search && (
            <button onClick={() => setSearch("")} style={{ background: "#f0f0f0", border: "none", borderRadius: 6, width: 24, height: 24, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", color: "#888" }}>
              {Icon.close}
            </button>
          )}
        </div>
        ...
        {/* ── Error ── */}
        {error && (
          <div style={{ background: C.red.bg, color: C.red.text, border: `0.5px solid ${C.red.border}`, borderRadius: 10, padding: "10px 16px", fontSize: 13, fontWeight: 600 }}>
            {error}
          </div>
        )}
            ...
            {catKeys.map((cat, ci) => {
              ...
              return (
                <div key={cat} style={{ background: "#fff", border: "0.5px solid #e8e8e8", borderRadius: 14, overflow: "hidden" }}>
                  ...
                  {/* Category header */}
                  <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "12px 20px", borderBottom: "0.5px solid #f0f0f0", background: "#FAFAFA" }}>
                    <div style={{ width: 28, height: 28, borderRadius: 7, background: col.bg, color: col.icon, display: "flex", alignItems: "center", justifyContent: "center" }}>
                      {Icon.tag}
                    </div>
                  ...
                  {grouped[cat].map((item, i) => (
                    <div key={item._id} className="item-row" style={{
                      display: "flex", alignItems: "center", padding: "13px 20px", gap: 14,
                      borderBottom: i < grouped[cat].length - 1 ? "0.5px solid #f5f5f5" : "none",
                      background: "#fff", transition: "background 0.12s",
                    }}>
                      <div style={{ width: 36, height: 36, borderRadius: 9, background: col.bg, color: col.icon, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        {Icon.box}
                      </div>
                      ...
                      <div style={{ background: col.bg, color: col.text, borderRadius: 20, padding: "4px 14px", fontSize: 13, fontWeight: 700, whiteSpace: "nowrap", flexShrink: 0 }}>
