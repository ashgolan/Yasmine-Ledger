        ...
        archive.append(
            toBuffer({
                ...
                files: [
                    ...
                    "quotes.json", "deliveryNotes.json",
                ],
            }),
            { name: "_index.json" }
        );
...
export function startWeeklyBackupJob() {
    ...
    // Every Sunday at 08:00 Jerusalem time
    // Format: second(optional) minute hour day-of-month month day-of-week
    cron.schedule(
        "0 8 * * 0",
        async () => {
            console.log("🔄 Weekly backup job started...");
            try {
                ...
                for (const user of users) {
                    ...
                    await transporter.sendMail({
                        ...
                        to: recipientEmail,
                        subject: `📦 גיבוי שבועי אוטומטי — ${dateStr}`,
                        html: `
              <div dir="rtl" style="font-family:Arial,sans-serif;max-width:480px;margin:auto">
                <h2 style="color:#534AB7">גיבוי שבועי אוטומטי ✅</h2>
                <p>שלום,</p>
                ...
                <p style="color:#888;font-size:13px">
                  הגיבוי נשלח אוטומטית כל יום ראשון בשעה 08:00.<br/>
    ...
    console.log("📅 Weekly backup job scheduled — every Sunday at 08:00 (Jerusalem)");
}

// للفحص فقط — استدعها من server.js مرة واحدة
export async function sendTestBackup() {
    console.log("🧪 Sending test backup email...");
    try {
        const users = await User.find({ isActive: true }).lean();
        const transporter = createTransporter();
        const zipBuffer = await generateBackupZip(users[0]._id);
        const dateStr = new Date().toLocaleDateString("he-IL").replace(/\//g, "-");

        await transporter.sendMail({
            from: `"Yasmine Ledger" <${process.env.EMAIL_USER}>`,
            to: process.env.BACKUP_EMAIL,
            subject: `🧪 טסט גיבוי — ${dateStr}`,
            text: "זהו מייל בדיקה לגיבוי האוטומטי",
            attachments: [{ filename: `test_backup.zip`, content: zipBuffer, contentType: "application/zip" }],
        });
        console.log("✅ Test backup email sent!");
    } catch (err) {
